...
public class TextToXmlPart {
	...
	public void buildUp() {
		try {
			// create & open files
			inLabel = new BufferedReader(new FileReader("wiki-dataset/wiki_labels_part.txt"));
			inLink = new BufferedReader(new FileReader("wiki-dataset/wiki_links_part.txt"));
			inAbst = new BufferedReader(new FileReader("wiki-dataset/wiki_abstracts_part.txt"));
			out = new StreamResult("wiki-dataset/wiki_data_part.xml");
			...
			while ((strLink = inLink.readLine()) != null && (strLabel = inLabel.readLine()) != null) {
				...
				List<String> vals_links = new ArrayList<String>();
				List<String> vals_labels = new ArrayList<String>();
