...
public class Main {
	...
	private static void moreOptions() {
		...
		System.out.println("1.Redo data preprocessing & Indexing (Full version)");
		System.out.println("2.Redo data preprocessing & Indexing (Partial version)");
		System.out.println("3.Reindex wikipedia Articles (Full version)");
		System.out.println("4.Reindex wikipedia Articles (Partial version)");
	...
	private static void reIndexAll() {
		try {
			...
			ArticleIndexerAll indexer = new ArticleIndexerAll();
	...
	/**
	 * Rebuild the PARTIAL version of the index. (always overwrite the old)
	 */
	private static void reIndexPart() {
		try {
			System.out.println("**********************");
			System.out.println("* Rebuilding Indexes *");
			System.out.println("**********************");
			...
			// reindex
			long startTime = System.nanoTime();
			ArticleIndexerPart indexer = new ArticleIndexerPart();
			indexer.rebuildIndexes();
			long estimatedTime = System.nanoTime() - startTime;
			double seconds = (double) estimatedTime / 1000000000.0;
	 ...
	 * 
	 * @param all
	 *            Flag to determine which version to be done (parial vs. full)
	 */
	private static void preProcessData(boolean all) {
		String[] args = {};
		if (all)
			TextToXmlAll.main(args);
		else
			TextToXmlPart.main(args);
	}
